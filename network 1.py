...
# NEW: support for post-game "ready_post", "quit_post", and "force_menu"
...
class LANServer:
    ...
    # NEW: post-game support
    def broadcast_force_menu(self):
        """Force all clients back to main menu (host quit in Game Over)."""
        msg = {"type": "state", "data": {"type": "force_menu"}}
        with self._lock:
            for cl in self._clients.values():
                try: _send(cl["sock"], msg)
                except Exception: pass
        ...
        while self._running:
            ...
            for slot in slots:
                ...
                for m in _recv_lines(cl["sock"], cl["buf"]):
                    ...
                    # NEW post-game messages
                    elif mtype == "ready_post":
                        self._events.append({"type": "ready_post", "slot": slot})
                    elif mtype == "quit_post":
                        self._events.append({"type": "quit_post", "slot": slot})
...
class LANClient:
    ...
    # NEW post-game methods
    def send_ready_post(self):
        """Tell host we want to play again."""
        _send(self._sock, {"type": "ready_post"})

    def send_quit_post(self):
        """Tell host we are quitting the Game Over screen."""
        _send(self._sock, {"type": "quit_post"})
