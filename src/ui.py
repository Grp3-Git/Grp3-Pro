...
# NEW: pause sidebar (ESC), blur overlay, game-over average, quit on death,
#      lobby shows connected player names live
...
def _blur_surface(surf: pygame.Surface, strength: int = 6) -> pygame.Surface:
    """
    Simple box-blur approximation by downscaling then upscaling.
    strength controls the blur radius (higher = blurrier).
    """
    w, h = surf.get_size()
...
class UI:
    ...
    Public interface used by main.py:
        ui.lang
        ui.toggle_lang()
        ui.draw_username(surface, username_str, cursor_visible) -> str|None
        ui.draw_lobby(surface, local_ip, players, is_host, join_ip, join_cursor) -> str|None
        ui.draw_hud(surface, elapsed, kill_counts, lives, players_info)
        ui.draw_menu(surface) -> str|None
        ui.draw_death(surface, elapsed, kill_counts, players_info) -> str|None
        ui.draw_pause(surface, game_surface, is_host, is_lan,
                      restart_countdown) -> str|None
            returns: "resume"|"restart"|"lang"|"quit"|None
    """
    ...
    def draw_lobby(self, surface, local_ip: str, players: list,
        ...
        """
        players: list of dicts {slot, username, kills}
        Returns: "start" | "join" | "back" | "typing" | "lang" | None
        """
        overlay = pygame.Surface((SCREEN_W, SCREEN_H), pygame.SRCALPHA)
        ...
        for slot in range(LAN_MAX_PLAYERS):
            ...
            if p:
                name_surf = self._font_score.render(
                    f"P{slot + 1}  {p['username']}", True, color)
    ...
    def draw_hud(self, surface, elapsed: float, kill_counts: list,
        ...
        lives_str = f"{self.t('hud_lives')}: {'♥ ' * lives}"

        pad = 12
        for i, (text, col) in enumerate([(time_str, HUD_TEXT), (lives_str, HUD_TEXT)]):
            surf = self._font_hud.render(text, True, col)
            y    = 10 + i * 34
            bg   = pygame.Surface((surf.get_width() + pad * 2, 30), pygame.SRCALPHA)
            bg.fill((10, 10, 10, 160))
            surface.blit(bg,   (10, y + 50))
            surface.blit(surf, (10 + pad, y + 52))

        self._draw_scoreboard(surface, kill_counts, players_info)

    def _draw_scoreboard(self, surface, kill_counts: list, players_info: list):
        panel_w = 220
        ...
        for slot in range(LAN_MAX_PLAYERS):
            ...
            if kc > 0 or p is not None:
                ...
                if name:
                    rows.append((slot, name, kc))

        panel_h = 44 + len(rows) * 34 + 8
        ...
        total_surf = self._font_score.render(f"KILLS  {total:04d}", True, (220, 50, 50))
        ...
        for i, (slot, name, kc) in enumerate(rows):
            row_y = div_y + 6 + i * 34
            color = PLAYER_COLORS[slot % len(PLAYER_COLORS)]
            pygame.draw.rect(surface, color,
                             pygame.Rect(panel_x + 6, row_y + 4, 5, 22),
                             border_radius=2)
            display_name = name[:10] if len(name) > 10 else name
            name_surf = self._font_score.render(display_name, True, color)
            surface.blit(name_surf, (panel_x + 18, row_y + 6))
            kc_surf = self._font_score.render(f"{kc}", True, color)
            surface.blit(kc_surf,
                         (panel_x + panel_w - kc_surf.get_width() - 10, row_y + 6))

    # ------------------------------------------------------------------ #
    #  Main menu  (DO NOT MODIFY — preserved exactly)
    # ------------------------------------------------------------------ #
    def draw_menu(self, surface):
        ...
        title = self._font_large.render("ARENA RPG", True, YELLOW)
    ...
    def draw_death(self, surface, elapsed: float, kill_counts: list,
        ...
        total      = sum(kill_counts)
        total_surf = self._font_medium.render(
        ...
        # Average kills per player (always shown when all players are dead)
        n_players = max(1, len(players_info))
        avg       = total / n_players
        avg_surf  = self._font_small.render(
            f"{self.t('avg_label')}:  {avg:.1f}", True, (200, 200, 100))
        surface.blit(avg_surf, avg_surf.get_rect(center=(cx, row_start_y + len(active_slots) * 30 + 16)))
    ...
    def draw_pause(self, surface, game_surface: pygame.Surface,
        ...
        """
        Draws blur overlay + transparent right-side sidebar with pause options.
        game_surface: a copy of the current game frame (before overlay).
        Returns: "resume" | "restart" | "lang" | "quit" | None
        """
        # --- Blur the game frame ---
        ...
        sidebar_surf.fill((10, 10, 10, 160))   # semi-transparent
        ...
        # Define options: (key, label, color, show_condition)
        options = []
        options.append(("resume",  self.t("pause_resume"),  (40, 120, 60),  True))
        # Restart: solo → always; LAN → only host
        show_restart = (not is_lan) or is_host
        options.append(("restart", self.t("pause_restart"), (30, 70, 150),  show_restart))
        options.append(("lang",    self.t("pause_lang"),    (60, 60, 120),  True))
        options.append(("quit",    self.t("pause_quit"),    (100, 30, 30),  True))
        ...
        # Restart countdown notification (shown to non-host in LAN)
    ...
    # ------------------------------------------------------------------ #
    #  LAN Pause sidebar  (game keeps running behind it — NO blur)
    # ------------------------------------------------------------------ #
    def draw_pause_lan(self, surface, is_host: bool,
                       restart_countdown: int = -1):
        """
        Draws the transparent pause sidebar directly over the live game frame.
        No blur — the game world remains fully visible and active behind it.
        Returns: "resume" | "restart" | "lang" | "quit" | None
        """
        sidebar_w = 280
        sidebar_x = SCREEN_W - sidebar_w - 20
        ...
        sidebar_surf = pygame.Surface((sidebar_w, sidebar_h), pygame.SRCALPHA)
        sidebar_surf.fill((10, 10, 10, 180))
        pygame.draw.rect(sidebar_surf, (80, 80, 80, 220),
                         pygame.Rect(0, 0, sidebar_w, sidebar_h),
                         width=1, border_radius=10)
        surface.blit(sidebar_surf, (sidebar_x, sidebar_y))
        ...
        paused_surf = self._font_medium.render(self.t("paused"), True, YELLOW)
        surface.blit(paused_surf,
                     paused_surf.get_rect(center=(sidebar_x + sidebar_w // 2,
                                                   sidebar_y + 40)))
        pygame.draw.line(surface, (80, 80, 80),
                         (sidebar_x + 20, sidebar_y + 68),
                         (sidebar_x + sidebar_w - 20, sidebar_y + 68))
        ...
        options = [
            ...
            ("restart", self.t("pause_restart"),  (30, 70, 150),  is_host),
        ...
        # Countdown notification for non-host clients
        if restart_countdown >= 0:
            msg_surf = self._font_small.render(
                self.t("restart_countdown", n=restart_countdown),
                True, (255, 200, 50))
            surface.blit(msg_surf,
                         msg_surf.get_rect(center=(SCREEN_W // 2, SCREEN_H - 60)))
