...
class UI:
    ...
    Public interface used by main.py:
        ...
        ui.draw_hud(surface, elapsed, kill_counts, lives, players_info,
                    respawn_timers={}) — NEW: respawn_timers = {slot: seconds_left}
        ui.draw_death(surface, elapsed, kill_counts, players_info) -> action
    ...
    def draw_hud(self, surface, elapsed: float, kill_counts: list,
        ...
        """
        respawn_timers: {slot: seconds_remaining} — shows countdown next to
        dead player names in the scoreboard.
        """
    ...
    def _draw_scoreboard(self, surface, kill_counts: list,
        ...
        """
        Right-side panel:
         - Total kills in bold red (always)
         - Per-player: name + individual kills (colored)
         - Respawn countdown next to dead players
        """
        panel_w = 250
        ...
        for i, (slot, name, kc, is_dead, respawn_t) in enumerate(rows):
            ...
            # Respawn countdown below name (only when dead and timer active)
    ...
    def draw_death(self, surface, elapsed: float, kill_counts: list,
                   players_info: list,
                   is_lan: bool = False, is_host: bool = False,
                   ready_players: list = None,
                   play_again_countdown: int = -1):
        """
        ready_players : list of {slot, username} who pressed Play Again (LAN).
        play_again_countdown : seconds remaining before restart (-1 = not counting).
        Returns: "retry" | "quit" | "lang" | None
        """
        overlay = pygame.Surface((SCREEN_W, SCREEN_H), pygame.SRCALPHA)
        ...
        # ── Ready table (LAN only) ─────────────────────────────────────
        table_bottom_y = row_start_y + len(active_slots) * 30 + 52
        if is_lan and ready_players is not None:
            ready_title = self._font_small.render("— Play Again —", True, (180, 180, 180))
            surface.blit(ready_title, ready_title.get_rect(center=(cx, table_bottom_y)))
            for i, rp in enumerate(ready_players):
                slot  = rp["slot"]
                color = PLAYER_COLORS[slot % len(PLAYER_COLORS)]
                txt   = self._font_score.render(
                    f"P{slot+1}  {rp['username'][:12]}  ✓ Ready", True, color)
                surface.blit(txt, txt.get_rect(center=(cx, table_bottom_y + 28 + i * 26)))
            table_bottom_y += 28 + len(ready_players) * 26 + 4

        # ── Play-Again countdown (LAN) ─────────────────────────────────
        if play_again_countdown >= 0:
            cd_surf = self._font_medium.render(
                f"Beginning in {play_again_countdown}…", True, (255, 210, 50))
            surface.blit(cd_surf, cd_surf.get_rect(center=(cx, table_bottom_y + 14)))
        ...
        retry_label = self.t("retry")
        self._draw_button(surface, retry_label, self._retry_btn_rect,
                          self._font_medium, bg=(40, 120, 60), fg=WHITE)
    ...
    def draw_pause(self, surface, game_surface: pygame.Surface,
        ...
        if restart_countdown >= 0:
            # Full-screen prominent countdown — visible even with pause menu open
            cd_bg = pygame.Surface((SCREEN_W, 70), pygame.SRCALPHA)
            cd_bg.fill((0, 0, 0, 160))
            surface.blit(cd_bg, (0, SCREEN_H // 2 - 35))
            msg_surf = self._font_large.render(
                f"Beginning in {restart_countdown}…", True, (255, 210, 50))
            surface.blit(msg_surf,
                         msg_surf.get_rect(center=(SCREEN_W // 2, SCREEN_H // 2)))
